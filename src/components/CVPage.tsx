...
const CVPage = () => {
  ...
  const languages = [
    { name: "Français", levelKey: "native", proficiency: 100 },
    { name: "Anglais", levelKey: "professional", proficiency: 85 },
    { name: "Espagnol", levelKey: "intermediate", proficiency: 60 },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 text-white">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <Link to="/" className="flex items-center gap-2 group">
              ...
              <span className="font-mono text-sm">{t("cvpage.back")}</span>
            </Link>
            ...
            <div className="lg:col-span-1 space-y-8">
                  ...
                  {languages.map((lang) => (
                    <div key={lang.name} className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="text-xs text-muted-foreground">{t("cvpage.lang_" + lang.levelKey)}</span>
                      </div>
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {['tech_watch', 'cybersecurity', 'ai_ml', 'blockchain', 'photography', 'travel'].map(function(key) {
                    return (
                      <span key={key} className="px-3 py-1 text-xs rounded-full bg-primary/10 text-primary border border-primary/20">
                        {t("cvpage.interest_" + key)}
                      </span>
                    );
                  })}
                </div>
                    ...
                    <div key={project.title} className="p-4 rounded-xl border border-white/10 bg-white/5">
                      ...
                      <div className="flex items-center justify-between">
                        <span className={`text-xs px-2 py-1 rounded-full ${
                          ...
                          {project.status === 'Terminé' ? t("cvpage.project_status_done") : t("cvpage.project_status_dev")}
                        </span>
      ...
      <footer className="py-8 border-t border-white/10 text-center">
        <p className="text-sm text-muted-foreground">
          © {new Date().getFullYear()} SIAKOU Komi Stanislas • {t("cvpage.footer")} {new Date().toLocaleDateString(i18n.language === 'en' ? 'en-US' : 'fr-FR')}
          <span className="mx-2">•</span>
          <Coffee className="inline w-4 h-4 mx-1" />
          {t("cvpage.footer_passion")}
        </p>
      </footer>
    </div>
  );
